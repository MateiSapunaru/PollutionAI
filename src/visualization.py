...
class SoilVisualization:
    ...
    def ph_bioavailability_analysis(self, save=False):
        ...
        for metal in ['Lead', 'Cadmium', 'Mercury', 'Chromium']:
            ...
            axes[0, 0].scatter(data['Soil_pH'], data['Bioavail_%'], label=metal, alpha=0.6, s=30)
        ...
        axes[0, 1].scatter(arsenic['Soil_pH'], arsenic['Bioavail_%'], color='darkred', alpha=0.6, s=30)
        ...
        ph_categories = pd.cut(self.df['Soil_pH'], bins=[4, 5.5, 7, 8.5], labels=['Acidic\n(<5.5)', 'Neutral\n(5.5-7)', 'Alkaline\n(>7)'])
        self.df['pH_Category'] = ph_categories

        cationic_cat = self.df[self.df['Pollutant_Type'].isin(['Lead', 'Cadmium', 'Mercury', 'Chromium'])].copy()
        cationic_cat['Bioavail_%'] = (cationic_cat['Bioavailable_Concentration_mg_kg'] / cationic_cat['Total_Concentration_mg_kg'] * 100)
        ...
        plt.tight_layout()
        if save:
            plt.savefig(os.path.join(self.output_dir, 'ph_bioavailability_analysis.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def soil_texture_analysis(self, save=False):
        ...
        axes[0, 0].bar(range(len(texture_bioavail)), texture_bioavail.values, color=colors, edgecolor='black')
        ...
        axes[1, 0].set_title('Disease Severity Distribution by Soil Texture', fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        if save:
            plt.savefig(os.path.join(self.output_dir, 'soil_texture_analysis.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def distance_decay_analysis(self, save=False):
        ...
        axes[0, 0].scatter(industrial['Distance_from_Source_km'], industrial['Total_Concentration_mg_kg'], alpha=0.5, s=30, color='darkblue')
        ...
        if mask.sum() > 10:
            ...
            axes[0, 0].plot(x_fit, np.exp(p(x_fit)), 'r--', linewidth=2, label=f'Exponential Fit')
            axes[0, 0].legend()
        
        industrial['Distance_Category'] = pd.cut(industrial['Distance_from_Source_km'], bins=[0, 1, 3, 5, 100], labels=['0-1 km', '1-3 km', '3-5 km', '>5 km'])
        
        dist_conc = industrial.groupby('Distance_Category')['Total_Concentration_mg_kg'].mean()
        axes[0, 1].bar(range(len(dist_conc)), dist_conc.values, color=['red', 'orange', 'yellow', 'green'], edgecolor='black')
        ...
        for industry in ['Mining', 'Battery', 'Tannery', 'Chemical']:
            ...
            if len(ind_data) > 5:
                axes[1, 0].scatter(ind_data['Distance_from_Source_km'], ind_data['Total_Concentration_mg_kg'], label=industry, alpha=0.6, s=40)
        ...
        severity_dist.plot(kind='bar', stacked=True, ax=axes[1, 1], color=['lightgreen', 'gold', 'orange', 'red'])
        ...
        plt.tight_layout()
        if save:
            plt.savefig(os.path.join(self.output_dir, 'distance_decay_analysis.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def pollutant_analysis(self, save=False):
        ...
        axes[0, 0].bar(pollutant_counts.index, pollutant_counts.values, color='steelblue', edgecolor='black')
        ...
        plt.tight_layout()
        if save:
            plt.savefig(os.path.join(self.output_dir, 'pollutant_analysis.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def age_vulnerability_analysis(self, save=False):
        ...
        plt.tight_layout()
        if save:
            plt.savefig(os.path.join(self.output_dir, 'age_vulnerability_analysis.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def industry_contamination_patterns(self, save=False):
        ...
        industry_severity.plot(kind='bar', ax=axes[1, 1], color=['lightgreen', 'gold', 'orange', 'red'])
        ...
        plt.tight_layout()
        if save:
            plt.savefig(os.path.join(self.output_dir, 'industry_patterns.png'), dpi=300, bbox_inches='tight')
        plt.show()
